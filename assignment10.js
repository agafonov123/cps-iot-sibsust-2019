...
var board = new firmata.Board("/dev/ttyACM0", function(){
    ...
    console.log("Enabling analog Pins 0, 1");
    board.pinMode(0, board.MODES.ANALOG); // analog pin 0
    board.pinMode(1, board.MODES.ANALOG); // analog pin 1
    board.pinMode(2, board.MODES.OUTPUT); // direction of DC motor
    board.pinMode(3, board.MODES.PWM);    // PWM of motor i.e. speed of rotation
    board.pinMode(4, board.MODES.OUTPUT); // direction DC motor
    board.pinMode(13,board.MODES.OUTPUT); // out-of-range indicator
});
...
var pwm;
var factor = 0.7; // proportional factor that determines the speed of aproaching toward desired value
...
function controlAlgorithm () {
    ...
    if(actualValue > 850 || actualValue < 147) {
        stopControlAlgorithm ();
...
function startControlAlgorithm () {
    if (controlAlgorithmStartedFlag == 0) {
        ...
        board.digitalWrite(13,0); // indicator OFF
